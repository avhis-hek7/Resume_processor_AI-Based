...
async function userRegisterController(req, res) {
  ...
  const isUserExists = await userModel.findOne({
    $or: [{ username }, { email }], // if one of the condition is satisfed then retun user{either thorugh email or username}
  });
...
async function userLoginController(req, res) {
  ...
  const isValidPassword = await isExistUser.comparePassword(password);
  if (!isValidPassword) {
    return res.status(400).json({
